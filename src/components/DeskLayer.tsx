...
function WaterBottle() {
  return (
    <div className="absolute z-[15] flex flex-col items-center" style={{ left: "2.5%", bottom: "32%" }}>
      ...
      {/* Body */}
      <div
        className="rounded-b-lg relative overflow-hidden flex items-center justify-center"
      ...
      >
        ...
        <p className="text-[5px] text-sky-800/70 font-black tracking-tight rotate-90 leading-none select-none whitespace-nowrap">
          MY STUDY ROOM
        </p>
      </div>
      {/* Base */}
...
function MascotOnDesk({ emoji, charMsg }: { emoji: string; charMsg: string }) {
  return (
    <div className="absolute z-[15] flex flex-col items-center" style={{ left: "10%", bottom: "29%" }}>
      ...
      {/* Character emoji */}
      <div className="drop-shadow-lg" style={{ fontSize: "clamp(1.8rem, 3vw, 3rem)" }}>
        {emoji}
      </div>
    </div>
  );
...
function StudyNotebook({ goal, finished }: { goal: string; finished: boolean }) {
  return (
    ...
    >
      ...
      {/* Notebook body */}
      <div
        className="bg-amber-50/97 rounded-r-xl shadow-2xl ring-1 ring-amber-200/50 flex flex-col"
      ...
      >
        ...
        <p className="text-slate-700 leading-snug break-words font-medium"
...
function Eraser() {
  return (
    <div
      ...
      style={{ right: "27%", bottom: "31%", transform: "rotate(-4deg)" }}
    >
      <div
        className="rounded flex overflow-hidden shadow"
      ...
      <p className="text-center text-[5px] font-black text-pink-100/90 absolute inset-0 flex items-center justify-center leading-none tracking-tight">
        MONO
      </p>
    </div>
  );
...
function PencilCase() {
  return (
    <div
      className="absolute z-[15]"
      style={{ right: "7%", bottom: "31%" }}
    >
      <div
        className="rounded-2xl shadow-xl relative overflow-hidden flex flex-col items-center justify-center"
      ...
      >
        ...
        <p className="relative text-amber-800/80 font-black tracking-tight" style={{ fontSize: "clamp(7px, 0.8vw, 10px)" }}>
          Keep going!
        </p>
      </div>
    </div>
...
function StickyNote() {
  return (
    <div
      className="absolute z-[15] flex items-center justify-center shadow-lg"
      style={{
        right: "6%",
        bottom: "35%",
...
export default function DeskLayer({ goal, characterId, charMsg, finished }: DeskLayerProps) {
  ...
  return (
    <>
      ...
      >
        {/* Top highlight edge */}
        <div className="absolute top-0 left-0 right-0 h-[3px]" style={{ background: "linear-gradient(90deg, rgba(255,255,255,0.08), rgba(255,255,255,0.18), rgba(255,255,255,0.08))" }} />
        <WoodGrains />
      </div>

      {/* Desk items */}
      <WaterBottle />
      <MascotOnDesk emoji={finished ? "🎉" : emoji} charMsg={charMsg} />
