...
export default function WeeklyRecap({ records }: WeeklyRecapProps) {
  ...
  if (weekRecords.length === 0) {
    return (
      <div className="p-5 bg-white rounded-2xl border border-stone-100">
        <p className="text-xs text-stone-400 mb-2">今週のふりかえり</p>
  ...
  return (
    <div className="p-5 bg-white rounded-2xl border border-stone-100 flex flex-col gap-4">
      <p className="text-xs text-stone-400">今週のふりかえり</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-stone-50 rounded-xl p-4">
          ...
          <p className="text-xs text-stone-400 mt-1">声を出せた日</p>
        </div>
        <div className="bg-stone-50 rounded-xl p-4">
          <p className="text-xl font-semibold text-stone-700">{formatSec(totalSec)}</p>
          <p className="text-xs text-stone-400 mt-1">合計の発話時間</p>
        </div>
      </div>

      <div>
        <p className="text-xs text-stone-400 mb-2">練習したカテゴリ</p>
        <div className="flex flex-col gap-1">
          {sortedCats.map(([cat, cnt]) => (
            <div key={cat} className="flex items-center justify-between px-3 py-2 bg-stone-50 rounded-lg">
              <span className="text-sm text-stone-600">{cat}</span>
              <span className="text-sm font-medium text-rose-500">{cnt}回</span>
            </div>
          ))}
        </div>
      ...
      <p className="text-sm text-stone-400 leading-relaxed bg-stone-50 rounded-xl px-4 py-3">
