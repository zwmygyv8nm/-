...
import { LocationVisualConfig, VisualConfigOverride, resolveVisualConfig } from "@/lib/study/storage";
...
export default function TravelStudyRoomScreen({
  ...
  const [startSeconds, setStartSeconds] = useState(25 * 60);
  ...
  /* 計算値 */
  const elapsedSec  = started ? startSeconds - secondsLeft : 0;
  ...
  const handleEnd    = useCallback(() => {
    onComplete(Math.max(1, Math.round((startSeconds - secondsLeft) / 60)));
  }, [startSeconds, secondsLeft, onComplete]);
